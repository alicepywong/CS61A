...
def pick(paragraphs, select, k):
    ...
    for i in range(k, len(paragraphs)):
        if select(paragraphs[i]):
            if i != k:
                tmp = paragraphs.pop(i)
                paragraphs.insert(k, tmp)
            return paragraphs[k]
    return ''
    # END PROBLEM 1
...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def keyword(words):
        wordList = words.translate(str.maketrans(
...
def accuracy(typed, reference):
    ...
    if typed_words == reference_words:
        return 100.0
    elif (typed_words == reference_words) or (not typed_words and reference_words) or (typed_words and not reference_words):
        return 0.0
    else:
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    diff_dict = {word: diff_function(typed_word, word, limit)
                 for word in word_list}
    diff_dict_sorted = dict(
        sorted(diff_dict.items(), key=lambda item: item[1]))

    return list(diff_dict_sorted.keys())[0] if list(diff_dict_sorted.values())[0] <= limit else typed_word
    # END PROBLEM 5
...
def feline_fixes(typed, reference, limit):
    ...
    if limit < 0:
        ...
        return len(typed)
    elif not typed and not reference:
        return 0
...
def hidden_kittens(typed, reference, limit):
    ...
    def helper(s1, s2):
        ...
        if cnt < 0:
            ...
            return helper(s1[1:], s2[1:]) + helper(s1[1:], s2)
        else:
            return helper(s1[1:], s2)

    res = helper(typed, reference)
    return limit + 1 if res == 0 else res
    # END PROBLEM 7
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i]:
            ...
            break
    accuracy = cnt / len(prompt)
    progress = {'id': user_id, 'progress': accuracy}
    upload(progress)
    return accuracy
    # END PROBLEM 8
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    words, players = match['words'], match['times']
    # initialization
    res = []
    ...
    for i in range(len(words)):
        ...
        for j in range(len(players)-1):
            index = min(index, players[j+1][i])
        for k in range(len(players)):
            if index == players[k][i]:
                res[k].append(words[i])
                break
