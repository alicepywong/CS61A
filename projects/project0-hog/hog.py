...
def pigs_on_prime(player_score, opponent_score):
    ...
    if not is_prime(player_score):
        ...
        player_pt = 1
        while not is_prime(player_score + player_pt):
            player_pt += 1
        return player_pt
    # END PROBLEM 4
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if last_leader == cur_leader or cur_leader == None:
        leader_message = None
    else:
        leader_message = f"Player {cur_leader} takes the lead by {abs(score0 - score1)}"

    return cur_leader, leader_message
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    max_scoring, max_scoring_num = 0, 1
    for i in range(1, 11):
        avg_dice = make_averaged(roll_dice, total_samples)
        cur_scoring = avg_dice(i, dice)
        if cur_scoring > max_scoring:
            max_scoring = cur_scoring
            max_scoring_num = i
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if pigs_on_prime(tmp + score, opponent_score) != 0 or tmp >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
