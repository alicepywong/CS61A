    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container:
                ...
                if self.ant_contained != place.ant:
                    assert place.ant is None, 'Two ants in {0}'.format(place)
            else:
                assert place.ant is None, 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place.bees == [] or self.lower_bound > step:
            place, step = place.entrance, step + 1
            if place.is_hive or step > self.upper_bound:
                return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        total = amount + self.damage if self.health <= 0 else amount
        for b in bees:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew != 0:
            ...
            target = random_bee(list(self.place.bees))
            if target:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def construct(cls, gamestate):
        ...
        if not gamestate.queenAnt:
            queen = super(QueenAnt, gamestate).construct(gamestate)
            ...
            if ant:
                if ant.is_container:
                    if ant.ant_contained and ant.ant_contained is not self:
                        ...
                        ant = None
                if ant not in self.ants_damage_doubled:
                    ant.double()
                    self.ants_damage_doubled.append()
            place = place.exit
        # END Problem 12
